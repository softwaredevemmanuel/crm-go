...
func (s *LiveClassService) validateLiveClass(req models.LiveClassInput) error {
    ...
    expectedDuration := int(req.EndTime.Sub(req.StartTime).Minutes())
    if req.Duration > 0 && req.Duration != expectedDuration {
        // Auto-calculate duration if not provided or mismatched
        req.Duration = expectedDuration
    }
    ...
    // Validate recurrence
    if req.RecurrenceType != "none" && req.RecurrenceEndDate != nil {
        if req.RecurrenceEndDate.Before(req.StartTime) {
            return errors.New("recurrence end date must be after start time")
        }
    }
...
func (s *LiveClassService) CreateLiveClass(req models.LiveClassInput) (*models.LiveClassResponse, error) {
    ...
    liveClass := models.LiveClass{
        ...
        RecordingRetention: req.RecordingRetention,
        CreatedAt:          time.Now(),
        UpdatedAt:          time.Now(),
    }
    ...
    if req.RecurrenceType != "none" && req.RecurrenceEndDate != nil {
        go s.createRecurringInstances(liveClass)
    }
...
func (s *LiveClassService) CreateLiveClassWithTx(tx *gorm.DB, req models.LiveClassInput) (*models.LiveClassResponse, error) {
    ...
    // Set defaults
    if req.Platform == "" { req.Platform = "zoom" }
    if req.AccessLevel == "" { req.AccessLevel = "enrolled" }
    if req.Timezone == "" { req.Timezone = "UTC" }
    if req.RecurrenceType == "" { req.RecurrenceType = "none" }
    if req.RecordingStorage == "" { req.RecordingStorage = "platform" }
...
func (s *LiveClassService) liveClassToResponse(liveClass *models.LiveClass, withDetails bool) *models.LiveClassResponse {
    response := &models.LiveClassResponse{
        ...
        Slug:               liveClass.Slug,
        ShortDescription:   liveClass.ShortDescription,
        StartTime:          liveClass.StartTime,
        EndTime:            liveClass.EndTime,
        Duration:           liveClass.Duration,
        Timezone:           liveClass.Timezone,
        RecurrenceType:     liveClass.RecurrenceType,
        RecurrenceRule:     liveClass.RecurrenceRule,
        RecurrenceEndDate:  liveClass.RecurrenceEndDate,
        ...
        RequiresApproval:   liveClass.RequiresApproval,
        ...
        RecordingRetention: liveClass.RecordingRetention,
...
// Create recurring instances (runs in background)
func (s *LiveClassService) createRecurringInstances(parentClass models.LiveClass) {
    // This is a simplified version - implement based on recurrence rules
    // Could use a job queue like Redis or RabbitMQ
    
    fmt.Printf("Creating recurring instances for class: %s\n", parentClass.Title)
    // Implementation depends on recurrence rules library
}