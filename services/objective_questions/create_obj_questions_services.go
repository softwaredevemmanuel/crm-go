...
// Check if question already exists (for same course, lesson)
func (s *ObjectiveQuestionService) questionExists(courseID, lessonID uuid.UUID, questionText string) (bool, error) {
	var count int64
	query := s.db.Model(&models.ObjectiveQuestion{}).
		Where("course_id = ? AND lesson_id = ? AND question_text = ?", courseID, lessonID, questionText)
